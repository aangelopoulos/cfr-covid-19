reindex_time <- function(data,L,min.cases) {
  ...
  assert("ERROR: There is no point in the data where N.1 > min.cases. Lower min.cases (and incur more error) or choose an area with more cases.", (first.t!=Inf) & (last.t!=-Inf))
  ...
  assert("ERROR: The length of the death distribution is long compared to the data. Choose a shorter DESIRED_L (and make sure MTTD is in the range 1:DESIRED_L).", (last.t-first.t) >= (L-1))
