...
print(getwd())
mat = npyLoad("./numpy_data/mat.npy")
colnames(mat) <- c("time", "grp", "R", "D", "N")
rownames(mat) <- 1:80
T = 40
min.cases <- 10
N.1 <- simulated.outbreak.deaths[1:T, "N"]
N.2 <- simulated.outbreak.deaths[(T+1):120, "N"]
...
simulated.outbreak.deaths[c(idx.for.Estep, idx.for.Estep + T), "new.times"] <- rep(new.times, + 2)
